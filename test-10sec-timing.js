...
async function test10SecTiming() {
  console.log('⏱️  Testing 10-second agent execution timing...\n');
  ...
  console.log(`📊 Expected: ~10000ms, Actual: ${duration}ms`);
  console.log(`🎯 Success: ${result.success}`);
  console.log(`📄 Generated: ${result.artifacts[0]?.name}`);
  
  const tolerance = 500; // 500ms tolerance
  if (Math.abs(duration - 10000) <= tolerance) {
    console.log('✅ Timing is correct!');
  } else {
    console.log('⚠️  Timing seems off');
  }
}
