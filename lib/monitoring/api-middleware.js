...
export function createMonitoringMiddleware() {
  return async (request, response, next) => {
    ...
    const captureResponse = (statusCode) => {
      ...
      healthMonitor.recordApiResponseTime(endpoint, method, responseTime, statusCode);
      
      // Record error if status code indicates an error
      if (statusCode >= 400) {
        const errorType = statusCode >= 500 ? 'server_error' : 'client_error';
        healthMonitor.recordError(endpoint, method, errorType, `HTTP ${statusCode}`);
      }
    };
    ...
    try {
      ...
      const responseTime = Date.now() - startTime;
      healthMonitor.recordApiResponseTime(endpoint, method, responseTime, 500);
      healthMonitor.recordError(endpoint, method, 'exception', error.message);
      throw error;
    }
  };
}
...
export function monitorAsync(name, fn) {
  return async (...args) => {
    ...
    try {
      ...
      healthMonitor.recordApiResponseTime(`internal/${name}`, 'INTERNAL', duration, 200);
      return result;
    } catch (error) {
      const duration = Date.now() - startTime;
      healthMonitor.recordApiResponseTime(`internal/${name}`, 'INTERNAL', duration, 500);
      healthMonitor.recordError(`internal/${name}`, 'INTERNAL', 'exception', error.message);
      throw error;
    }
  };
}
