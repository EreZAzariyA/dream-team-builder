...
export class HealthMonitor {
  ...
  recordApiResponseTime(endpoint, method, responseTime, statusCode) {
    ...
    // Keep only last 100 entries per endpoint (reduced from 1000)
  ...
  async getHealthStatus() {
    ...
    // Get latest metrics
    let latestDbMetrics = Array.from(this.metrics.databaseMetrics.entries())
      .sort(([a], [b]) => b - a)[0]?.[1];
    
    let latestSysMetrics = Array.from(this.metrics.systemMetrics.entries())
      .sort(([a], [b]) => b - a)[0]?.[1];
    ...
    if (!latestDbMetrics || !latestSysMetrics) {
      ...
      try {
        // Collect database metrics
        if (!latestDbMetrics) {
          latestDbMetrics = await this.collectDatabaseMetrics();
          this.metrics.databaseMetrics.set(now, latestDbMetrics);
        }
        
        // Collect system metrics
        if (!latestSysMetrics) {
          latestSysMetrics = await this.collectSystemMetrics();
          this.metrics.systemMetrics.set(now, latestSysMetrics);
        }
      } catch (error) {
   ...
   */
  cleanupOldMetrics() {
    const cutoffTime = new Date(Date.now() - 2 * 60 * 60 * 1000); // 2 hours ago (reduced from 24 hours)
  ...
  clearMetrics() {
    ...
    this.metrics.databaseMetrics.clear();
    this.metrics.systemMetrics.clear();
    
    logger.info('All monitoring metrics cleared');
