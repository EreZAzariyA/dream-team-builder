...
export const queryClient = new QueryClient({
  defaultOptions: {
    queries: {
      // Cache Configuration
      staleTime: 30 * 1000, // 30 seconds - shorter stale time to reduce excessive calls
      ...
      retry: (failureCount, error) => {
        ...
        // Retry only once for other errors to reduce server load
      ...
      refetchOnReconnect: false,   // Disable refetch on reconnect to reduce calls
      refetchOnMount: 'always',    // Only refetch if data is stale
      refetchInterval: false,      // Disable automatic background refetching globally
...
// Background sync utilities - DISABLED to reduce server load
export const enableBackgroundSync = () => {
  // Background sync disabled to prevent excessive API calls
  // Individual components should manage their own refresh intervals if needed
  console.log('Background sync disabled to optimize performance');
};
