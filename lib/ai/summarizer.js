...
export async function generateAISummary(analysis, fileIndex, metrics, userId) {
  ...
  try {
    ...
    prompt += `Top Files:\n${topFiles}\n\n`;
    prompt += `**IMPORTANT: You have access to readFile tool to examine repository files!**

**How to use readFile tool:**
- Call it with the "path" parameter: readFile({ path: "package.json" })
- You can read files like: package.json, README.md, tsconfig.json, src/index.ts, etc.
- Read 3-5 important files to provide accurate code quality assessment
- Examples:
  * readFile({ path: "package.json" }) - Check dependencies
  * readFile({ path: "README.md" }) - Understand project purpose
  * readFile({ path: "tsconfig.json" }) - Check TypeScript configuration
  * readFile({ path: "src/app.ts" }) - Examine main application code

**Please read key files BEFORE writing your analysis to ensure accuracy!**
\n\n`;
...
- Output ONLY Markdown, no preamble or closing remarks`;
    ...
    logger.info(`🤖 Generating AI summary with Analyst agent (with file access tools): ${JSON.stringify({
    ...
    // Set up repository context for tools
    const toolExecutor = await import('../ai/tools/toolExecutor.js');
    ...
    // Call AI service with tools enabled so agent can read files
    const response = await aiService.callWithTools(prompt, analystAgent, 3, {
