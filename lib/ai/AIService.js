...
export class AIService {
  constructor() {
    ...
    this.geminiTimeout = 30000; // Default 30 second timeout
  ...
  async initializeClients(apiKeys) {
    ...
    if (apiKeys.gemini) {
      try {
        ...
        // Configure reasonable timeout for Gemini requests
        this.geminiTimeout = 30000; // 30 seconds timeout
  ...
  async callGemini(prompt, agent, complexity, context, userId) {
    ...
    try {
      ...
      const timeout = this.geminiTimeout || 30000;
      logger.info(`🔍 [GEMINI] Making API call to Gemini Pro with ${timeout}ms timeout`);
      
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(() => {
          logger.error(`⏰ [GEMINI] API request timed out after ${timeout}ms`);
