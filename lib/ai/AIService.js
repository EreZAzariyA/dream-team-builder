...
export class AIService {
  ...
  async initialize(userApiKeys = null, userId = null) {
    try {
      ...
      if (!apiKeys) {
        apiKeys = this.loadApiKeysFromEnvironment();
      }

      if (!apiKeys || (!apiKeys.gemini && !apiKeys.openai)) {
        logger.warn('No API keys found. AI service will run in limited mode.');
        return false;
      }
  ...
  loadApiKeysFromEnvironment() {
    ...
    if (process.env.OPENAI_API_KEY) {
      envKeys.openai = process.env.OPENAI_API_KEY;
    }
    if (process.env.GEMINI_API_KEY) {
      envKeys.gemini = process.env.GEMINI_API_KEY;
    }
    
    if (envKeys.openai || envKeys.gemini) {
  ...
  async callGemini(prompt, agent, complexity, context, userId) {
    ...
    try {
      ...
      if (userId) {
        ...
        const tokens = usage?.totalTokenCount || text.length / 4;
        const cost = this.estimateGeminiCost(tokens);
        ...
        logger.info('📊 Gemini actual usage:', {
