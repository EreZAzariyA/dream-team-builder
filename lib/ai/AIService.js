...
export class AIService {
  ...
  generateFallbackResponse(prompt, agent) {
    // NO FALLBACK RESPONSES - AI service must work with user keys
    throw new Error('All AI providers failed - no fallback response will be generated. Please ensure your API keys are configured.');
  }
  ...
  async streamResponse(prompt, agent = null, complexity = 1, context = {}, userId = null, options = {}) {
    ...
    try {
      ...
      const streamResult = await this.aiSdkService.streamText(prompt, {
        ...
        maxTokens: Math.min(4000, 2000 * complexity),
