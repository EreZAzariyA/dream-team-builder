...
export class AIService {
  ...
  async callWithTools(prompt, agent, complexity, context, userId) {
    ...
    logger.info(`🛠️ Starting tool-enabled AI call for user: ${userId || 'anonymous'}`);
    ...
    try {
      ...
      // Ensure AISdkService has current API keys
      if (this.currentApiKeys && (this.currentApiKeys.gemini || this.currentApiKeys.openai)) {
        logger.info(`🔑 Updating AISdkService with keys: gemini=${!!this.currentApiKeys.gemini}, openai=${!!this.currentApiKeys.openai}`);
        this.aiSdkService.updateApiKeys(this.currentApiKeys);
      }

      // Use the AI SDK's generate function with tools
      ...
      if (userId) {
        this.usageTracker.trackUsage(userId, 'tools', {
          provider: 'auto',
          complexity,
  ...
  async callGemini(prompt, agent, complexity, context, userId) {
    ...
    try {
      ...
      const complexityValue = typeof complexity === 'string' ? 
        (complexity === 'simple' ? 1 : complexity === 'complex' ? 4 : 2) : 
        complexity;
      const maxTokens = Math.min(8000, 2000 * complexityValue); // Increased base tokens to prevent truncation
      ...
      logger.info('🔍 [GEMINI DEBUG] Full response structure:', {
      ...
      if (response.candidates && response.candidates.length > 0) {
        ...
        logger.info('🔍 [GEMINI DEBUG] First candidate details:', {
      ...
      try {
        text = response.text();
        logger.info('🔍 [GEMINI DEBUG] Text extraction successful:', {
