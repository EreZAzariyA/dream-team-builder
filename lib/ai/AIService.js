...
export class AIService {
  ...
  async call(prompt, agent = null, complexity = 1, context = {}, userId = null, useTools = false) {
    ...
    if (!this.initialized && userId) {
      ...
      if (!initSuccess) {
        throw new Error(`AI Service initialization failed: No API keys configured for user ${userId}. Please configure your API keys in user settings before running workflows.`);
      }
    } else if (!this.initialized) {
      throw new Error('AI Service initialization failed: No API keys configured. Please provide API keys.');
    }
