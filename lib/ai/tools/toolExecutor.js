  ...
  execute: async (toolCall, userId = null) => {
    ...
    try {
      ...
      if (toolImplementations[toolName]) {
        ...
        if (['createBranch', 'deleteBranch', 'createOrUpdateFile', 'createCommit', 'readFile', 'switchWorkingBranch', 'getWorkflowStatus', 'listBranches', 'createPullRequest', 'getRepositoryInfo'].includes(toolName)) {
          ...
          // Add working branch as head branch for pull requests
          if (toolName === 'createPullRequest' && currentWorkingBranch && !args.head) {
            enhancedArgs.head = currentWorkingBranch;
            logger.info(`🌿 Using working branch as PR head: ${currentWorkingBranch}`);
          }
        }
        ...
        logger.info(`✅ Tool execution completed: ${toolName} - result: ${typeof result === 'object' ? JSON.stringify(Object.keys(result)) : result}`);
        const toolResult = { toolName, result, success: true };
        logger.info(`🔍 Returning tool result: ${JSON.stringify(toolResult)}`);
        return toolResult;
      } else {
