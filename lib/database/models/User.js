...
UserSchema.methods.setApiKeys = function(apiKeys) {
  logger.info('🔑 setApiKeys called with:', { hasOpenai: !!apiKeys.openai, hasGemini: !!apiKeys.gemini });
  ...
  if (apiKeys.openai) {
    logger.info('🔐 Setting OpenAI key (first 10 chars):', apiKeys.openai.substring(0, 10));
    this.apiKeys.openai = encrypt(apiKeys.openai); // Encrypt before storing
  }
  
  if (apiKeys.gemini) {
    logger.info('🔐 Setting Gemini key (first 10 chars):', apiKeys.gemini.substring(0, 10));
    this.apiKeys.gemini = encrypt(apiKeys.gemini); // Encrypt before storing
  }
  
  this.apiKeys.updatedAt = new Date();
  logger.info('💾 Final apiKeys object before save:', {
