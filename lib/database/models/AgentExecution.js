...
const AgentExecutionSchema = new mongoose.Schema({
  ...
  output: {
    ...
    artifacts: [{
      ...
      type: {
        type: String,
        enum: ['document', 'code', 'image', 'data', 'report', 'template'],
      },
  ...
  messages: [{
    ...
    type: {
      type: String,
      enum: ['input', 'output', 'inter_agent', 'system', 'error', 'debug'],
      required: true,
    },
    fromAgent: String,
    toAgent: String,
...
AgentExecutionSchema.methods.addMessage = function(messageData) {
  const message = {
...
AgentExecutionSchema.methods.addArtifact = function(artifactData) {
  if (!this.output.artifacts) {
  ...
  this.output.artifacts.push(artifact);
  return this.save();
};
