...
import { pusherClient } from './config';
...
const logger = {
  info: (...args) => console.log('[Pusher]', ...args),
  warn: (...args) => console.warn('[Pusher]', ...args),
  error: (...args) => console.error('[Pusher]', ...args),
};
...
export class BmadPusherClient {
  ...
  async connect() {
    ...
    logger.info('🔄 Attempting to connect to Pusher...', {
    ...
    return new Promise((resolve, reject) => {
      ...
      pusherClient.connection.bind('connected', () => {
        logger.info('🌐 Connected to Pusher successfully!');
        this.connected = true;
        this.connecting = false;
      ...
      pusherClient.connection.bind('state_change', (states) => {
        logger.info('🔄 Pusher state change:', states.previous, '→', states.current);
      });

      // If already connected, resolve immediately
      if (pusherClient.connection.state === 'connected') {
        logger.info('✅ Pusher already connected!');
        this.connected = true;
        this.connecting = false;
        this.statistics.connectionTime = new Date();
        this.emit('connected');
      ...
      if (pusherClient.connection.state === 'initialized') {
        logger.info('🔄 Starting Pusher connection...');
  ...
  disconnect() {
    ...
    // Clean up connection event listeners to prevent memory leaks
  ...
  subscribeToWorkflow(workflowId) {
    ...
    // Add missing user and agent message handlers
    ...
    logger.info(`📡 Subscribed to workflow: ${workflowId}`);
    return true;
  }
  ...
  unsubscribeFromWorkflow(workflowId) {
    ...
    if (channel) {
      ...
      logger.info(`📡 Unsubscribed from workflow: ${workflowId}`);
    }
  ...
  subscribeToAgent(agentId) {
    ...
    logger.info(`📡 Subscribed to agent: ${agentId}`);
    return true;
  }
  ...
  unsubscribeFromAgent(agentId) {
    ...
    if (channel) {
      pusherClient.unsubscribe(channelName);
      this.subscriptions.delete(channelName);
      logger.info(`📡 Unsubscribed from agent: ${agentId}`);
    }
  ...
  async broadcastMessage(content, target) {
    try {
      ...
      if (response.ok) {
        ...
        logger.info('📤 Message sent via Pusher');
        return true;
      } else {
