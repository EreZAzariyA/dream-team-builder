  ...
  useEffect(() => {
    const checkProviders = () => {
      ...
      try {
        ...
        // Pusher is now used instead of WebSocket manager
        newStatus.webSocket = true; // Always true since Pusher handles this
...
export const useProviderHealth = () => {
  ...
  useEffect(() => {
    const runHealthCheck = () => {
      ...
      // Pusher health check (always healthy since it's managed externally)
      checks.webSocket = { status: 'healthy', message: 'Pusher real-time messaging ready' };
      passedChecks++;
