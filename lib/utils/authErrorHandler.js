...
class AuthErrorHandler {
  ...
  /**
   * Enhanced console.error that respects auth error throttling with debugging info
   */
  logAuthError(message, endpoint, debugInfo = {}, ...args) {
    if (this.shouldReportAuthError(endpoint)) {
      ...
      console.error(`[AUTH ERROR] ${message}`, enhancedError);
    }
    // Silent when throttled
  }

  /**
   * Log session recovery events
   */
  logSessionRecovery(method, details = {}) {
    console.info(`[SESSION RECOVERY] Method: ${method}`, {
      timestamp: new Date().toISOString(),
      ...details
    });
  }
