...
// Prevent multiple logger initialization in development hot reloads
if (global.dreamTeamLogger) {
  logger = global.dreamTeamLogger;
} else {
  ...
  try {
    ...
    logger = winston.createLogger({
      ...
      transports,
      exitOnError: false,
      handleExceptions: false,  // Prevent duplicate listeners - handled at transport level
      handleRejections: false,  // Prevent duplicate listeners - handled at transport level
      silent: false,
      defaultMeta: { 
        service: 'dream-team',
  ...
  // Store logger globally to prevent re-initialization in hot reloads
  global.dreamTeamLogger = logger;
}
