...
class BmadOrchestrator {
  constructor(store = null, options = {}) {
    ...
    this.artifactManager = new GitHubArtifactManager(options.gitService);
  ...
  async startWorkflow(userPrompt, options = {}) {
    ...
    try {
      ...
      if (options.githubContext || options.teamContext?.githubIntegration?.enabled) {
        ...
        logger.info(`🐙 GitHubArtifactManager initialized for team-based workflow: ${githubConfig.repository.full_name || githubConfig.repository.owner + '/' + githubConfig.repository.name}`);
      }
