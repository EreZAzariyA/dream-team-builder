...
class BmadOrchestrator {
  ...
  async initialize() {
    try {
      ...
      console.log(`Initializing BMAD Orchestrator${modeText}...`);
      ...
      if (message.workflowId) {
        ...
        this.triggerPusherEvent(message.workflowId, 'workflow-message', {
          message: {
            ...
            content: this.truncateContentForPusher(message.content), // Truncate content for Pusher
            timestamp: new Date().toISOString()
          }
        });
   ...
   */
  /**
   * Safely truncate content for Pusher events (handles strings, objects, arrays)
   */
  truncateContentForPusher(content) {
    if (!content) return null;
    
    try {
      // If it's an object, create a summary version
      ...
      console.error('Error truncating content for Pusher:', error);
