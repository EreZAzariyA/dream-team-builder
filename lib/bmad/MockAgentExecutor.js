...
class MockAgentExecutor {
  ...
  async executeAgent(agent, context) {
    ...
    try {
      logger.info(`🤖 [MOCK] Starting ${agent.id} agent execution...`);
      ...
      logger.info(`✅ [MOCK] ${agent.id} completed successfully`);
  ...
  generateAnalystContent(projectName) {
    return {
...
`
  ...
  generateArchitectContent(projectName) {
    return {
...
`
  ...
  generateDevContent(projectName) {
    return {
...
`
  ...
  generateGenericContent(agent, projectName) {
    return {
