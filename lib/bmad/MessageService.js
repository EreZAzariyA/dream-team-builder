...
class MessageService {
  ...
  async loadFromAgentMessageCollection(workflowId) {
    try {
      ...
      // Find the workflow document first to get the correct ObjectId
      const workflow = await this.findWorkflow(workflowId);
      if (!workflow) {
        return []; // No workflow found, return empty array
      }
      
      // Use the workflow's _id (ObjectId) for the query
      const messages = await AgentMessage.find({ workflowId: workflow._id }).sort({ timestamp: 1 });
