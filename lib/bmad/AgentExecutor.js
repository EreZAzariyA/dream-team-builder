...
class AgentExecutor {
  ...
  async handleElicitationStep(agent, context) {
    ...
    try {
      // First, let the agent generate detailed elicitation text
      let agentGeneratedText = null;
      
      if (this.aiService) {
        const agentPrompt = `You are a ${agent.agent?.title || agent.agent?.name || agent.id} working on a project workflow.

The user described their project as: "${context.userPrompt || context.workflowContext?.userPrompt || 'User is working on a project'}"

Current workflow step: "${context.step || 'elicitation'}"
Context notes: "${context.stepNotes || context.notes || 'Gathering user input for workflow continuation'}"

Your job is to create detailed, comprehensive questions to gather the information you need from the user. Be thorough, ask for specific details, provide examples if helpful, and explain why you need this information.

Generate detailed questions to help gather the required information:`;

        logger.info('🤖 [AGENT GENERATION] Calling AI to generate detailed agent response');
        
        const userId = context.userId || context.workflowContext?.initiatedBy;
        try {
          const agentResponse = await this.aiService.call(agentPrompt, agent, 1, context, userId);
          
          if (agentResponse && agentResponse.content && agentResponse.content.trim().length > 0) {
            agentGeneratedText = agentResponse.content.trim();
            logger.info('🤖 [AGENT GENERATION] Agent generated detailed text:', {
              textLength: agentGeneratedText.length,
              textPreview: agentGeneratedText.substring(0, 300) + (agentGeneratedText.length > 300 ? '...' : '')
            });
          }
        } catch (error) {
          logger.warn('⚠️ [AGENT GENERATION] Failed to generate agent text:', error.message);
        }
      }
