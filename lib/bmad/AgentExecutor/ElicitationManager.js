...
class ElicitationManager {
  ...
  async generateElicitationQuestions(agent, context) {
    try {
      ...
      const response = await aiService.call(prompt, agent, 1, context, context.userId);
...
Response:`;
  ...
  async generateBasicProjectBriefResponse(userInput, agent, context = {}) {
    const { AIService } = await import('../../ai/AIService.js');
    const aiService = AIService.getInstance();
    ...
    const response = await aiService.call('', agent, 1, { userPrompt: userInput }, context.userId || null);
...
Response:`;
  ...
  async generateBasicGenericResponse(userInput, agent, templateName, context = {}) {
    const { AIService } = await import('../../ai/AIService.js');
    const aiService = AIService.getInstance();
    ...
    const response = await aiService.call('', agent, 1, { userPrompt: userInput, templateName }, context.userId || null);
