  ...
  async callAIService(prompt, agent, attempt, context = null) {
    ...
    if (!this.aiService.initialized) {
      ...
      if (userId) {
        ...
        try {
          // Use correct initialize signature: (apiKeys, userId)
          await this.aiService.initialize(null, userId);
          logger.info(`✅ [AI SERVICE] Successfully initialized AIService for user: ${userId}`);
        } catch (initError) {
          logger.error(`❌ [AI SERVICE] Failed to initialize AIService for user: ${userId}`, {
            error: initError.message
          });
          throw new Error(`AI Service initialization failed for user ${userId}. Error: ${initError.message}. Please configure your API keys in the user settings.`);
        }
      } else {
