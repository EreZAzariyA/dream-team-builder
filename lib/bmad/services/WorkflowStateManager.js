...
class WorkflowStateManager {
  ...
  async resumeWorkflowWithElicitation(workflowId, elicitationResponse, agentId, userId = null) {
    ...
    try {
      ...
      if (currentStep) {
        ...
        if (isDynamicWorkflow) {
          ...
          try {
            ...
            logger.info(`✅ [WORKFLOW STATE MGR] handleRegularStep completed successfully`);
          } catch (handleRegularStepError) {
            logger.error(`❌ [WORKFLOW STATE MGR] handleRegularStep failed:`, handleRegularStepError);
            throw handleRegularStepError;
          }
        } else {
      ...
      logger.error(`💥 [RESUME ELICITATION] Error resuming workflow ${workflowId} with elicitation:`, error);
      logger.error(`💥 [RESUME ELICITATION] Error stack:`, error.stack);
      logger.error(`💥 [RESUME ELICITATION] Error details:`, {
  ...
  async processElicitationResponseForRouting(workflow, elicitationResponse, agentId) {
    ...
    // Let agents naturally process user responses through their AI capabilities
    // Agents have built-in interaction handling via their activation-instructions and elicitation methods
    // Agents will naturally process responses based on their context and instructions
  }
