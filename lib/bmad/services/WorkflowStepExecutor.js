...
class WorkflowStepExecutor {
  ...
  async executeNextStep(workflowId) {
    ...
    try {
      ...
      // Phase 2: Centralized Elicitation Logic
        ...
        // Send message through communicator (for event handling)
        await this.engine.communicator.sendMessage(workflowId, {
          from: step.agentId,
          to: 'user',
          type: 'elicitation_request',
        ...
        // DIRECT FIX: Send Pusher event directly to ensure frontend gets the prompt
        await this.engine.pusherService.triggerElicitationRequest(workflowId, elicitationRequest);
        
        // Also send the interactive prompt as a workflow message
        await this.engine.pusherService.triggerWorkflowMessage(workflowId, {
          from: step.agentId,
          to: 'user',
          content: elicitationRequest.instruction,
          type: 'elicitation',
          agentId: step.agentId
        });
      ...
      if (!rollbackSuccess) {
        ...
        // CRITICAL FIX: Notify AgentTeam of workflow failure
        await this.notifyTeamDeploymentFailure(workflowId, errorMessage, {
          step: workflow.currentStep,
          agent: workflow.currentAgent,
  ...
  prepareAgentContext(workflow, step, agent) {
    return {
      ...
      // CRITICAL FIX: Include all step data for interactive template detection
      action: step.action,
      ...
      // CRITICAL FIX: Add userId for AIService auto-initialization during workflow execution
  ...
  /**
   * CRITICAL FIX: Notify AgentTeam when workflow fails
   * This bridges the gap between WorkflowEngine (in-memory) and AgentTeam (database)
   */
  async notifyTeamDeploymentFailure(workflowId, errorMessage, details = {}) {
    try {
      ...
      if (teamDeployment) {
        ...
        await teamDeployment.fail(`Workflow execution failed: ${errorMessage}`, {
          workflowFailure: true,
          workflowId: workflowId,
          failedAt: 'workflow_execution',
          ...details
        });

        console.log(`🔗 [WorkflowStepExecutor] Team deployment ${teamDeployment.teamInstanceId} marked as failed due to workflow error`);
      } else {
        console.warn(`⚠️ [WorkflowStepExecutor] No team deployment found for failed workflow ${workflowId}`);
      }
    } catch (notificationError) {
      // Don't throw - notification failure shouldn't break the main workflow error
      console.error('❌ [WorkflowStepExecutor] Failed to notify team deployment of workflow failure:', notificationError.message);
    }
  }
}
