...
class WorkflowStepExecutor {
  ...
  async executeAIWork(agent, step, context, workflow) {
    ...
    let aiResponse;
    try {
      aiResponse = await this.aiService.call(agentPrompt, {
        agent,
        complexity: 3, // 'complex' maps to complexity level 3
        context,
      ...
      const recoveryResult = await this.errorRecoveryManager.handleError(aiError, {
        ...
        context,
        workflow,
        retryCallback: () => this.aiService.call(agentPrompt, {
          agent,
          complexity: 3,
          context,
          userId: workflow.userId
      ...
      if (recoveryResult.success && recoveryResult.requiresRetry) {
        ...
        const complexity = recoveryResult.strategy === 'use_fallback_model' ? 1 : 3;
        aiResponse = await this.aiService.call(agentPrompt, {
          agent,
          complexity,
          context: recoveryResult.adjustedContext || context,
          userId: workflow.userId
        });
