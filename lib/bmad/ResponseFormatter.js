...
class ResponseFormatter {
  ...
  static formatAgentResponse(agentDefinition, rawContent, metadata = {}) {
    ...
    if (isConversationalMode) {
      ...
      return {
        ...
        response: {
          type: 'conversation',
          summary: `${agent.name || 'Agent'} responds`,
          content: {
    ...
    // Check if this is an elicitation response (should be passed through unmodified)
    const isElicitation = this.isElicitationResponse(rawContent);
    
    if (isElicitation) {
      // For elicitation responses, preserve the rich formatting exactly as the agent generated it
      return {
        agentId: agent.id || 'unknown',
        agentName: agent.name || 'Agent',
        agentTitle: agent.title || 'Assistant',
        ...
        response: {
          type: 'elicitation',
          summary: `${agent.name || 'Agent'} is requesting information`,
          content: {
            main: rawContent, // Pass through unmodified
            sections: []
          },
          codeModifications: []
        },
  ...
  static isElicitationResponse(content) {
    // Detect elicitation responses that should be preserved unmodified
    const elicitationIndicators = [
      // Agent introductions
      // /Hello,?\s+I'm\s+\w+.*(?:Architect|PM|Analyst|Developer)/i,
      
      // Common elicitation phrases
      /(?:need|require|want|would like)(?:.*?)(?:you to (?:provide|describe|tell|explain))/i,
      /(?:please|can you)(?:.*?)(?:provide|describe|tell|explain)/i,
      /(?:I need|We need|Could you)(?:.*?)(?:details|information|specifics)/i,
      
      // Structured question patterns
      /\*\s*What is.*\?/,  // Bullet point questions
      /including:\s*\n.*\*/s,  // "including:" followed by bullet points
      
      // Service analysis specific patterns
      /(?:service|project|system).*(?:enhance|improve|modify)/i,
      /existing.*(?:service|system|codebase)/i,
      
      // Question lists
      /(?:\n\s*\*|\n\s*-).*\?.*(?:\n\s*\*|\n\s*-).*\?/s  // Multiple questions in bullet format
    ];
    
    // Check if content matches elicitation patterns
    return elicitationIndicators.some(pattern => pattern.test(content));
  }
}
