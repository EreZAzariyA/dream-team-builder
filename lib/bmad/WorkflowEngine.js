...
class WorkflowEngine {
  ...
  prepareAgentContext(workflow, step, agent) {
    ...
    logger.info('🔍 [WORKFLOW ENGINE DEBUG] prepareAgentContext step data:');
    logger.info(`  - stepId: ${step.id}`);
    logger.info(`  - stepName: ${stepName}`);
    logger.info(`  - agentId: ${step.agentId}`);
    logger.info(`  - action: ${step.action}`);
    logger.info(`  - hasNotes: ${!!stepNotes}`);
    logger.info(`  - notesLength: ${stepNotes?.length}`);
    logger.info(`  - notesPreview: ${stepNotes?.substring(0, 300)}`);
    logger.info(`  - allStepKeys: [${Object.keys(step).join(', ')}]`);
    logger.info(`🔍 [WORKFLOW ENGINE DEBUG] step.notes full content: ${stepNotes}`);
    logger.info(`🔍 [WORKFLOW ENGINE DEBUG] CRITICAL - stepNotes being passed to agent: ${stepNotes}`);
    ...
    return {
      ...
      stepNotes: stepNotes  // ✅ FIXED - Use correct field with fallbacks
    };
  }
