...
class WorkflowEngine {
  ...
  async initialize() {
    try {
      ...
      console.log(`BMAD Workflow Engine initialized successfully${modeText}`);
      return true;
    } catch (error) {
  ...
  async executeNextStep(workflowId) {
    ...
    try {
      ...
      // Handle execution result - CRITICAL: Ensure sequential execution
      if (executionResult.success || executionResult.timedOut) {
        ...
        // Agent failed completely - stop workflow execution
        console.error(`Agent ${step.agentId} failed completely. Stopping workflow.`);
        workflow.status = WorkflowStatus.ERROR;
        workflow.errors.push({
          timestamp: new Date(),
        ...
        return; // Stop execution, do not proceed to next agent
      }
  ...
  async handleAgentCompletion(workflowId, agentId, executionResult) {
    ...
    console.log(`🔄 Handling completion for agent ${agentId} in workflow ${workflowId}`);
    ...
    // Store execution results only if they exist
    ...
    if (!executionResult.success) {
      if (executionResult.timedOut) {
        ...
        // Non-timeout error
        console.error(`Agent ${agentId} failed with error: ${executionResult.error}`);
        workflow.errors.push({
          timestamp: new Date(),
    ...
    // CRITICAL: Only proceed to next step if this agent is the current agent
    if (workflow.currentAgent !== agentId) {
      console.warn(`⚠️ Agent ${agentId} completed but is not the current agent (${workflow.currentAgent}). Skipping next step.`);
      return;
    }
    ...
    console.log(`✅ Agent ${agentId} completed. Moving to step ${workflow.currentStep}/${workflow.sequence.length}`);

    // Execute next step or complete workflow - ENSURE SEQUENTIAL EXECUTION
    if (workflow.currentStep < workflow.sequence.length) {
      console.log(`🔄 Proceeding to next agent in sequence...`);
      await this.executeNextStep(workflowId);
    } else {
      console.log(`🎉 All agents completed. Finalizing workflow...`);
      await this.completeWorkflow(workflowId);
    }
  }
