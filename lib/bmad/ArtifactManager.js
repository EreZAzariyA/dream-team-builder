...
class ArtifactManager {
  ...
  async initialize() {
    if (!isServer) {
      logger.warn('ArtifactManager can only run on server side');
      return false;
    }
    ...
    try {
      ...
      logger.info('BMAD Artifact Manager initialized');
      return true;
    } catch {
      logger.error('Failed to initialize Artifact Manager:', error);
      throw error;
    }
  }
  ...
  async saveWorkflowArtifacts(workflowId, artifacts) {
    if (!isServer || !this.initialized) {
      logger.warn('Cannot save artifacts - server environment required');
      return [];
    }
    ...
    try {
      ...
      logger.info(`Saved ${savedArtifacts.length} artifacts for workflow ${workflowId}`);
      return savedArtifacts;

    } catch (error) {
      logger.error(`Error saving artifacts for workflow ${workflowId}:`, error);
      throw error;
    }
  }
  ...
  async saveArtifact(workflowDir, artifact) {
    try {
      ...
      if (!validateFilePath(filepath)) {
        throw new Error(`Invalid or unsafe artifact file path: ${filepath}`);
      }
      ...
      logger.info(`Saved artifact: ${filename}`);
      return savedArtifact;

    } catch (error) {
      logger.error(`Error saving artifact ${artifact.name}:`, error);
      return null;
    }
  }
  ...
  async createWorkflowManifest(workflowDir, workflowId, artifacts) {
    ...
    logger.info(`Created workflow manifest: ${manifestPath}`);
  }
  ...
  async loadWorkflowArtifacts(workflowId) {
    ...
    try {
      ...
      try {
        ...
        logger.info(`No artifacts found for workflow ${workflowId}`);
        return [];
      }
      ...
      for (const artifactInfo of manifest.artifacts) {
        ...
        try {
          ...
          logger.warn(`Could not load artifact ${artifactInfo.filename}:`, error);
        }
      }
      ...
      logger.error(`Error loading artifacts for workflow ${workflowId}:`, error);
      return [];
    }
  }
  ...
  async getWorkflowList() {
    ...
    try {
      ...
      for (const entry of entries) {
        if (entry.isDirectory()) {
          ...
          try {
            ...
            logger.warn(`Could not load manifest for workflow ${workflowId}`);
          }
        }
      }
      ...
      logger.error('Error getting workflow list:', error);
      return [];
    }
  }
  ...
  async deleteWorkflowArtifacts(workflowId) {
    ...
    try {
      ...
      logger.info(`Deleted artifacts for workflow ${workflowId}`);
      return true;
    } catch (error) {
      logger.error(`Error deleting artifacts for workflow ${workflowId}:`, error);
      return false;
    }
  }
  ...
  async exportWorkflowArtifacts(workflowId, format = 'json') {
    ...
    try {
      ...
      logger.error(`Error exporting artifacts for workflow ${workflowId}:`, error);
      return null;
    }
  }
  ...
  async removeDirectory(dirPath) {
    try {
      ...
      logger.warn(`Could not remove directory ${dirPath}:`, error);
    }
  }
  ...
  /**
   * Get workflow artifacts (alias for loadWorkflowArtifacts)
   */
  async getWorkflowArtifacts(workflowId) {
    return await this.loadWorkflowArtifacts(workflowId);
  }
  ...
  async getArtifactContent(workflowId, filename) {
    ...
    try {
      const workflowDir = path.join(this.artifactsPath, 'workflows', workflowId);
      const artifactPath = path.join(workflowDir, filename);
      
      // Check if file exists
      try {
        await fs.access(artifactPath);
      } catch (error) {
        logger.info(`Artifact ${filename} not found for workflow ${workflowId}`);
        return null;
      }
      ...
      logger.error(`Error getting artifact content for ${filename}:`, error);
      return null;
    }
  }
  ...
  async cleanup(olderThanDays = 30) {
    ...
    try {
      ...
      logger.info(`Cleaned up ${cleanedCount} old workflow artifact directories`);
      return cleanedCount;
