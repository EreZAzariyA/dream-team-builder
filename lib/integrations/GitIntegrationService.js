...
// Global singleton cache for initialized plugins to prevent repeated initialization
// This prevents the rate limit issue by reusing the same plugin instance
...
export class GitIntegrationService {
  constructor(userContext = null) {
    ...
    // Create cache key based on user token - each user gets their own cached plugin
    const cacheKey = userContext?.githubAccessToken ? 
      userContext.githubAccessToken.substring(0, 10) : // First 10 chars of token
      `user-${userContext?.id || userContext?.email || 'anonymous'}`;
      
    // Use cached plugin if available - this prevents repeated initializations
    if (pluginCache.has(cacheKey)) {
      this.githubPlugin = pluginCache.get(cacheKey);
      this.initialized = true;
      logger.debug(`🔄 Reusing cached GitHubPlugin for user: ${cacheKey}`);
    } else {
      this.githubPlugin = new GitHubPlugin();
      this.initialized = false;
      logger.debug(`🆕 Creating new GitHubPlugin for user: ${cacheKey}`);
    }
  }
  ...
  async initialize(config = {}) {
    try {
      if (!this.initialized) {
        ...
        // Cache the initialized plugin using the same key strategy
        const cacheKey = this.userContext?.githubAccessToken ? 
          this.userContext.githubAccessToken.substring(0, 10) :
          `user-${this.userContext?.id || this.userContext?.email || 'anonymous'}`;
          
        pluginCache.set(cacheKey, this.githubPlugin);
        
        // Clean old cache entries (keep only last 20 for better performance)
        if (pluginCache.size > 20) {
          const firstKey = pluginCache.keys().next().value;
          pluginCache.delete(firstKey);
          logger.debug('🧹 Cleaned old plugin cache entry');
        }
        
        logger.info('🔧 Git Integration Service initialized (no connection test)');
      } else {
        logger.debug('🔧 Using cached Git Integration Service - no API calls needed');
      }
      return true;
    } catch (error) {
