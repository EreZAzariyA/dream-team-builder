...
export class GitHubPlugin extends BasePlugin {
  constructor() {
    ...
    // RATE LIMITING & CACHING: Implement intelligent API management
    this.cache = new Map();
    this.rateLimitInfo = {
      remaining: 5000,
    ...
    // Cache TTL settings (in milliseconds) - increased to reduce API calls
    this.cacheTTL = {
      user: 900000,        // 15 minutes (was 5) - connection tests are expensive
      repositories: 600000,  // 10 minutes (was 5) 
      repository: 900000,   // 15 minutes (was 10)
      branches: 600000,     // 10 minutes (was 5)
      commits: 300000,      // 5 minutes (unchanged - commits change frequently)
      contents: 900000      // 15 minutes (was 10)
    };
  ...
  /**
   * Test GitHub API connection - ONLY called when actually needed, with aggressive caching
   */
  async testConnection() {
    try {
      // Use cached connection test result if available and recent (15 minutes - much longer cache)
      const cacheKey = 'github-connection-test';
      const cachedResult = this.getFromCache(cacheKey, 'user');
      if (cachedResult) {
      ...
      // Cache the result for 15 minutes (much longer than before)
      this.setCache(cacheKey, user, 'user');
      ...
      if (error.status === 403 && error.message.includes('rate limit')) {
        ...
        const cachedResult = this.cache.get('github-connection-test');
        if (cachedResult) {
  ...
  /**
   * RATE LIMITING & CACHING: Check if cached data is still valid
   */
  ...
  /**
   * RATE LIMITING & CACHING: Get data from cache if valid
   */
  getFromCache(cacheKey, type = 'repository') {
    const cacheEntry = this.cache.get(cacheKey);
    const ttl = this.cacheTTL[type] || this.cacheTTL.repository;
    
    if (this.isCacheValid(cacheEntry, ttl)) {
  ...
  /**
   * RATE LIMITING & CACHING: Store data in cache
   */
  setCache(cacheKey, data, type = 'repository') {
    ...
    logger.debug(`💾 [GitHub Cache] Cached data for ${cacheKey}`);
  }
  ...
  async makeRateLimitedRequest(requestFn, cacheKey, type = 'repository') {
    // Check cache first
    const cachedData = this.getFromCache(cacheKey, type);
    if (cachedData) {
      return cachedData;
    }
    ...
    try {
      ...
      // Cache the result
      this.setCache(cacheKey, response.data, type);
