...
export class OpenAPIGenerator {
  ...
  async generatePaths(spec) {
    ...
    for (const routeFile of routeFiles) {
      ...
      try {
        ...
        console.warn(`Error parsing route file ${routeFile}:`, error.message);
      }
    }
  }
   ...
   */
  generateMethodDoc(method, routePath, content) {
      ...
      for (const modelFile of modelFiles) {
        if (modelFile.endsWith('.js')) {
          ...
          try {
            ...
            console.warn(`Error parsing model ${modelFile}:`, error.message);
          }
        }
      }
    } catch (error) {
      console.warn('Error reading models directory:', error.message);
    }
  ...
  parseMongooseSchema(content, modelName) {
    try {
      ...
      console.warn(`Error parsing schema for ${modelName}:`, error.message);
      return null;
    }
  }
  ...
  async saveSpec(spec, outputPath = 'docs/openapi.json') {
    ...
    try {
      ...
      console.log(`OpenAPI specification saved to ${fullPath}`);
      return fullPath;
    } catch (error) {
