  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Repository Insights</h2>
          {cacheInfo && (
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              {cacheInfo.cached ? (
                <>
                  <span className="text-green-600 dark:text-green-400">●</span> Loaded from {cacheInfo.source === 'redis' ? 'cache' : 'database'}
                </>
              ) : (
                <>
                  <span className="text-blue-600 dark:text-blue-400">●</span> Freshly generated by QA agent
                </>
              )}
            </p>
          )}
